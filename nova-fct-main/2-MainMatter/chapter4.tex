...
Chapter~\ref{cha:compiling}. Writing \texttt{create} as a record literal is therefore not an arbitrary convention but a syntactic
...
\texttt{prepare}/\texttt{effect} split from it, Chapter~\ref{cha:compiling}. Why3, in turn, uses \texttt{execute} directly, with no
...
automated proof obligations for VeriFx, Chapter~\ref{cha:compiling}, which is why this particular form is restricted to a fixed vocabulary
...
\texttt{.zip} and \texttt{.map}, used later when compiling \texttt{merge} and \texttt{compare} for this CRDT, Chapter~\ref{cha:compiling}.
