...
In this section, a collection of CRDTs implemented and verified in VeriFx is presented, corresponding to \todo{adicionar Listings}. These implementations cover both state-based and operation-based CRDTs, including counters and set data types, and illustrate how VeriFx is used to automatically verify convergence properties.
...
\begin{description}
      \item[Grow-only Counter] constitutes the basis for state-based counters. Its implementation, shown in Listing \todo{adicionar GCounter Listing} and provided by Kevin De Porre, follows the formal specification by Shapiro et al.\todo{adicionar citação de shapiro}, modeling the state as a \texttt{Vector[Int]} where each position corresponds to a specific replica.
      ...
      \item[Positive-Negative Counter] builds upon the previous definition to demonstrate the verification of composed CRDTs. Its implementation, shown in Listing \todo{adicionar PNCounter Listing}, structures the state by composing two GCounter instances: \texttt{p} to track increments, and \texttt{n} to track decrements.
...
\begin{description}
      \item[Grow-only Set] represents an append-only collection. Its implementation, shown in Listing \todo{adicionar GSet Listing}, models the state using a generic immutable set \texttt{Set[V]}, ensuring monotonicity by construction, as elements can be added but never removed.
...
Operation-Based OR-Set implements the Observed-Remove Set following the CmRDT model. Its implementation, shown in Listing \todo{adicionar ORSet Listing}, adapts Specification 15 from Shapiro et al.\todo{adicionar citação de shapiro} to VeriFx, ensuring Add-Wins semantics.
...
To illustrate the verification of CRDTs in Why3, the Grow-only and Positive-Negative Counters are translated into WhyML. These example demonstrates how components verified in VeriFx can be formalized within a deductive setting that supports explicit invariants and interactive reasoning. The following discussion analyzes the implementation in Listings \todo{adicionar why3 GCounter e PNCounter Listing}, comparing it with the reference code in Listing \todo{adicionar PNCounter Listing}.

\subsection{Abstract CRDT Module}
\label{sub:crdt_module}

In Why3, the CRDT module is established through a formal contract within an abstract module, contrasting with the traits used in VeriFx. The definition, shown in Listing \todo{adicionar CRDT module Listing}, declares abstract types for the internal state \texttt{t} and the observable value \texttt{payload}. The module specifies the core operations \texttt{merge} and \texttt{compare}, with the latter used to define state equivalence through mutual comparison in the \texttt{equals} predicate. Correctness properties are defined as axioms, such as \texttt{merge\_commutative}, which enforce convergence conditions at the specification level by requiring implementations to provide formal proofs.

\subsection{Grow-only Counter Module}
\label{sub:gcounter_whyml}

The generic \texttt{CRDT} interface is limited to merge and comparison operations and therefore does not include update operations such as \texttt{increment}. To work within this restriction, the \texttt{GCounter}, presented in Listing \todo{adicionar GCounter why3 Listing}, adopts a structure using two modules, with the update logic defined separately in an auxiliary module, \texttt{GCAuxiliary}.
